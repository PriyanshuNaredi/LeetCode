...
class Solution {
    ...
    MyNode getMaxSum(TreeNode* root) {
        ...
        if (!root)
            return MyNode(INT_MIN, INT_MAX, 0); // if it is null node always BST

        MyNode myLeftNode = getMaxSum(root->left);
        MyNode myRightNode = getMaxSum(root->right);

        if (myLeftNode.maxNode < root->val &&
            root->val < myRightNode.minNode) { // if it is BST

            MAX_SUM = max(MAX_SUM,
                          myLeftNode.maxSum + myRightNode.maxSum + root->val);

            return MyNode(max(root->val, myRightNode.maxNode),
